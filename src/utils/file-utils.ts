export default class FileUtils {
  ...
  static getWishListHeader(row) {
    ...
    return obj.map((key) => this.stringToVariableName(key));
  }
  static getWishListHeaderObject(row) {
    const obj = Object.keys(row);
    return obj.map((key) => this.stringToVariableName(key)).reduce((res, key, index) => {
      res[key] = obj[index];
      return res;
    }, {});
  }
  static stringToVariableName(str) {
